  ...
  return (
    <>
      <div className="flex flex-col pl-3 md:px-0 mt-5 overflow-y-scroll w-full gap-3 md:gap-2 mb-3">
        ...
        <div className="flex items-center justify-between gap-3 w-full flex-wrap">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            ...
            {/* Tambah Menu */}
            <button
              className="px-6 py-4 bg-secondary text-sm text-textColor rounded-md font-bold hover:opacity-90 cursor-pointer"
        ...
        <div className="flex flex-col gap-3">
          {/* Header */}
          <div className="grid grid-cols-[3fr_2fr_2fr_1.5fr_1fr_1.5fr] items-center px-5 py-4 bg-secondary text-solidThree font-semibold rounded-xl">
            <p className="ml-20 text-solidThree">Menu</p>
            <p className="text-solidThree">Kategori</p>
            <p className="ml-5 text-solidThree">Tipe</p>
          ...
          {products.map((product) => (
            <div
              key={product._id}
              className="grid grid-cols-[3fr_2fr_2fr_1.5fr_1fr_1.5fr] items-center px-5 py-4 bg-secondary rounded-xl gap-x-4"
            >
              {/* Menu */}
      ...
      {addMenuOpen && (
        ...
        >
          <div
            className="p-6 rounded-lg shadow-xl w-11/12 flexCenter md:w-260 relative"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Konten Modal */}
            <TambahMenu onClose={() => setAddMenuOpen(false)} />
          </div>
        </div>
      )}
      ...
      {editMenuOpen && (
        ...
        >
          ...
          >
            {/* Konten Modal */}
            <EditMenu onClose={() => setEditMenuOpen(false)} />
          </div>
        </div>
      )}
    </>
